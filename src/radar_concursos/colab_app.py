...
class RadarColabApp:
    ...
    def refresh(self) -> None:
        ...
        self.discoveries = self.data.get("discoveries", {}).get("discoveries", [])
    ...
    def _build_history_panel(self):
        ...
        self.h_sync_all = w.Button(description="Sincronizar todos os órgãos agora", button_style="primary", icon="cloud-download")
        self.h_remote_button = w.Button(description="Atualizar somente este órgão", button_style="info", icon="refresh")
        ...
        self.h_sync_all.on_click(self._history_sync_all)
        ...
        return w.VBox([
            ...
            self.h_auto_search, w.HBox([self.h_sync_all, self.h_remote_button]), self.h_remote_results, self.h_remote_save, self.h_remote_output,
    ...
    def _history_organ_changed(self, *_: object) -> None:
        ...
        auto_found = [x for x in self.discoveries if x.get("organ_id") == oid]
        auto_found.sort(key=lambda x: (x.get("year") or 0, x.get("last_seen_at") or ""), reverse=True)
        warning = f"<br><b>Faltam {shortage} concurso(s) estruturado(s) para completar o histórico mínimo.</b>" if shortage else ""
        discovery_html = ""
        if auto_found:
            rows = "".join(f"<li><a href='{html.escape((d.get('sources') or [{}])[0].get('url',''))}' target='_blank'>{html.escape(str(d.get('year') or '—'))} — {html.escape(d.get('title',''))}</a> <small>({html.escape(d.get('status',''))})</small></li>" for d in auto_found[:8])
            discovery_html = f"<br><b>{len(auto_found)} descoberta(s) automática(s):</b><ul>{rows}</ul>"
        self.h_summary.value = f"<div style='padding:12px;background:#f4f8f6;border-radius:10px'><b>{html.escape(organ.get('acronym',''))} — {html.escape(organ.get('name',''))}</b><br>{len(all_items)} concurso(s) estruturado(s) na base; exibindo {mode}.{warning}{discovery_html}</div>"
    ...
    def _history_sync_all(self, _: object) -> None:
        with self.h_remote_output:
            self.h_remote_output.clear_output()
            try:
                print("Sincronizando todos os órgãos cadastrados. Isso pode levar alguns minutos...")
                report = sync_organs(data_dir=self.data_dir, days=730, max_results=80)
                self.refresh()
                metrics = report.get("metrics", {})
                print(f"Órgãos consultados: {metrics.get('organs_requested', 0)}")
                print(f"Órgãos com ao menos um provedor bem-sucedido: {metrics.get('organs_succeeded', 0)}")
                print(f"Atualizações novas: {metrics.get('updates_added', 0)}")
                print(f"Descobertas novas: {metrics.get('discoveries_added', 0)}")
                print(f"Total de descobertas automáticas: {metrics.get('discoveries_total', 0)}")
                self._history_refresh_hierarchy()
            except Exception as exc:
                print("ERRO NA SINCRONIZAÇÃO AUTOMÁTICA:", exc)
    ...
    def _build_publish_panel(self):
        ...
        self.pub_publish = w.Button(description="Salvar direto no GitHub", button_style="primary", icon="cloud-upload")
        self.pub_export = w.Button(description="Exportar ZIP (backup)", button_style="warning")
        self.pub_output = w.Output(); self.pub_test.on_click(self._test_build); self.pub_preview.on_click(self._preview); self.pub_publish.on_click(self._publish_github); self.pub_export.on_click(self._export)
        return w.VBox([w.HTML("<h3>Gerar e publicar</h3><p>O botão <b>Salvar direto no GitHub</b> usa o Secret <code>GH_TOKEN</code> do Colab. Nenhum processamento ocorre no seu computador. O ZIP fica apenas como backup opcional.</p>"), w.HBox([self.pub_test, self.pub_preview, self.pub_publish, self.pub_export]), self.pub_output])
    ...
    def _publish_github(self, _: object) -> None:
        with self.pub_output:
            self.pub_output.clear_output()
            try:
                ...
                if not token:
                    raise ValueError("Crie o Secret GH_TOKEN no Colab (ícone de chave) com permissão Contents: Read and write para este repositório.")
                env = dict(os.environ); env["PYTHONPATH"] = str(self.root / "src")
                subprocess.run([sys.executable, "-m", "unittest", "discover", "-s", "tests", "-v"], cwd=self.root, env=env, check=True)
                build()
                subprocess.run(["git", "config", "user.name", "Osmar via Google Colab"], cwd=self.root, check=True)
                subprocess.run(["git", "config", "user.email", "actions@users.noreply.github.com"], cwd=self.root, check=True)
                subprocess.run(["git", "add", "-A"], cwd=self.root, check=True)
                diff = subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=self.root)
                if diff.returncode == 0:
                    print("Nenhuma alteração nova para publicar.")
                    return
                subprocess.run(["git", "commit", "-m", "chore: atualizar radar pelo Colab"], cwd=self.root, check=True)
                secure_url = f"https://x-access-token:{token}@github.com/osmarrcs/radar-concursos-ti.git"
                subprocess.run(["git", "push", secure_url, "HEAD:main"], cwd=self.root, check=True)
                print("Alterações enviadas ao GitHub. O GitHub Actions iniciará a publicação do portal.")
            except Exception as exc:
                print("ERRO AO PUBLICAR NO GITHUB:", exc)
